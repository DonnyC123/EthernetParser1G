...
class RxMacChecker(GenericChecker):
  ...
  def check_parser_flags(self, actual_out):
    if (1 < 
        int(actual_out.is_preamble_or_sfd_o) 
      + int(actual_out.is_dst_mac_o) 
      + int(actual_out.is_src_mac_o) 
      + int(actual_out.is_ether_type_o) 
      + int(actual_out.is_payload_or_crc_o)):
  ...
  async def check(self, expected_queue, actual_queue):
    ...
    while(not actual_queue.empty()):
      ...
      if actual_out.is_dst_mac_o:
        dst_mac += actual_out.rx_data_o.to_bytes(byteorder="big")
      if actual_out.is_src_mac_o:
        src_mac += actual_out.rx_data_o.to_bytes(byteorder="big")
      if actual_out.is_ether_type_o:
        eth_type += actual_out.rx_data_o.to_bytes(byteorder="big")
      if actual_out.is_payload_or_crc_o:
        payload += actual_out.rx_data_o.to_bytes(byteorder="big")
