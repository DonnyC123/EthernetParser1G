module ipv4_parser #(
  ...
  input   logic         clk,
  input   logic         rst,
  data_valid_if.slave   rx_data_if_i,
  eth_fields_if.slave   eth_fields_if_i,
  ip_fields_if.master   ip_fields_if_o
);
  ...
  always_comb begin
    ip_fields_if_o.idle           = parser_state_ff == IDLE_OR_VERSION_HLEN;
    ip_fields_if_o.src_addr       = parser_state_ff == SRC_IP;
    ip_fields_if_o.dst_addr       = parser_state_ff == DST_IP;
    ip_fields_if_o.option_or_data = parser_state_ff == OPTION_OR_DATA;
  end
